...
serve(async (req) => {
  try {
    ...
    const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(query)}&type=video&order=date&publishedAfter=${oneDayAgo}&key=${apiKey}&maxResults=20`;
